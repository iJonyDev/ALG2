...
public class Population {
    ...
    public Individual mutate(Individual ind) {
        ...
        ind.getTour().set(index1, ind.getTour().get(index2));
        ind.getTour().set(index2, storage);
        return ind;
    }
    ...
    // Búsqueda local del algoritmo memético
    public void LocalSearch() {
        int i;
        Individual c;
        for (i = 0; i < TSP.POPULATION_SIZE; i++) {
            c = individuals.get(i);
            individuals.set(i, findBetter(c));
        }
    }

    private Individual findBetter(Individual c) {
        Individual vecino = new Individual();
        Individual aux = new Individual();
        int i = 0;
        boolean encontrado = false;

        aux.setTour(c.getTour());
        aux.setCost(c.getCost());

        while (!encontrado && i < numCities) {
            vecino = mutate(aux);
            vecino.calculateCost();
            if (vecino.getCost() < c.getCost()) {
                encontrado = true;
            } else {
                i++;
            }
