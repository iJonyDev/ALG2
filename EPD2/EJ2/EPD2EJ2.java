...
public class EPD2EJ2 {
    ...
    public static Solucion tabuSearch() {
        ...
        while (iteraciones > 0 && existeMejor) {
            ...
            // Si el vecinoMejor tiene un movimiento de -1 es porque no se encontro ninguno
            // válido en el método anterior: parada
            ...
            // Actualizamos la solución actual con la del mejor vecino, para comenzar ahí en
    ...
    private static void generarVecinos(Solucion solActual, Solucion[] vecinos) {
        ...
        for (i = 0; i < NUM_VECINOS; i++) {
            ...
            vecinos[i].setMovimiento(bitObjetivo); // Almaceno el movimiento realizado, para pasarlo luego a la lista
                                                   // tabú
    ...
    private static void permuta(Solucion vecino, int i) {
        // Metodo para permutar un bit de la solución actual y convertirlo en un vecino
        // nuevo
    ...
    private static boolean criterioAspiracion(Solucion solMejor, Solucion vecino, Solucion vecinoMejor) {
        ...
        if (vecino.getCoste() > solMejor.getCoste() && vecino.getCoste() > vecinoMejor.getCoste()
                && vecino.getCoste() <= PESOMAX) { // Controlamos que no se haya encontrado el mejor entre los vecinos
                                                   // ya evaluados y que no se exceda del peso permitido
            aceptar = true;
        }
        return aceptar;
    }
